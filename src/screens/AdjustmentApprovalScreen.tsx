...
import { RootStackParamList, AdjustmentRequest, AdjustmentStatus } from '../types';
...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'AdjustmentApproval'>;
  route: RouteProp<RootStackParamList, 'AdjustmentApproval'>;
};

const FILTERS: { label: string; value: AdjustmentStatus | 'All' }[] = [
...
export const AdjustmentApprovalScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const [target, setTarget] = useState<AdjustmentRequest | null>(null);
  ...
  const load = async () => {
    try {
      const res = await api.getPendingAdjustments(token, statusFilter);
      if (res.success) setItems(res.adjustments);
    } catch (error: any) {
  ...
  const openModal = (adj: AdjustmentRequest, dec: 'Approved' | 'Rejected') => {
    setTarget(adj);
    setDecision(dec);
    setComment('');
    setShowDecide(true);
  ...
  const confirmDecision = async () => {
    ...
    setDeciding(true);
    try {
      const res = await api.decideAdjustment(token, target.id, decision, comment);
      if (res.success) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle}>Adjustment approvals</Text>
          <Text style={styles.headerSubtitle}>Review punch-correction requests</Text>
        </View>
      </View>
        ...
        {loading && !refreshing ? (
          ...
          items.map((adj) => (
            <ApprovalCard
              key={adj.id}
              adj={adj}
              onApprove={() => openModal(adj, 'Approved')}
              onReject={() => openModal(adj, 'Rejected')}
            />
          ))
        )}
        ...
        <View style={styles.modalScrim}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>
              {decision === 'Approved' ? 'Approve adjustment' : 'Reject adjustment'}
            </Text>
            {target && (
              <Text style={styles.modalSubtitle}>
                {target.employee_name} · {target.type} · {target.doc_no}
              </Text>
            )}
            ...
            <TextInput
              ...
              placeholder={decision === 'Approved' ? 'Note for the employee…' : 'Why rejected?'}
...
const ApprovalCard: React.FC<{
  ...
  return (
    <View style={styles.card}>
      ...
      <Text style={styles.cardName}>{adj.employee_name}</Text>
      <Text style={styles.cardMeta}>
        {adj.type} · {adj.attendence_date ? adj.attendence_date.split('T')[0] : '—'}
      </Text>
      ...
      {adj.manager_comment && adj.status !== 'Pending' && (
        <View style={styles.metaRow}>
